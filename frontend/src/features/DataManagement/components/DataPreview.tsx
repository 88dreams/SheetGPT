...
import { type ExtractedData } from '../hooks';
...
/**
 * Component that renders a preview of extracted data before saving
 */
            ...
            {extractedData.rows && extractedData.rows.map((row: any, index: number) => (
              <tr key={index}>
                {row.map((cell: any, cellIndex: number) => (
                  <td key={`${index}-${cellIndex}`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {cell}
                  </td>
                ))}
              </tr>
