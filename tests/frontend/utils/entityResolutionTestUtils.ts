...
/**
 * Setup mocks for entity resolver and API cache
 * @param resolverResults Optional custom resolver results
 * @returns Mock utilities and cleanup function
 */
export const setupEntityResolutionMocks = (resolverResults?: Record<string, any>) => {
  // Default resolver behavior
  const defaultResolver = (type: string, name: string) => {
    if (name === 'not found') {
      return null;
    }
    
    const matchType = name.includes('exact') ? 'exact' :
                     name.includes('fuzzy') ? 'fuzzy' :
                     name.includes('context') ? 'context' :
                     name.includes('virtual') ? 'virtual' : 'default';
                     
    return { 
      id: `${type}_${matchType}`, 
      name: `${matchType.charAt(0).toUpperCase() + matchType.slice(1)} ${type.charAt(0).toUpperCase() + type.slice(1)}`, 
      type: type
    };
  };
  
  // Default cache behavior
  const defaultCache = (key: string) => {
    if (key.includes('exact')) {
      return {
        resolution_info: mockResolutionInfo('exact')
      };
    } else if (key.includes('fuzzy')) {
      return {
        resolution_info: mockResolutionInfo('fuzzy')
      };
    } else if (key.includes('context')) {
      return {
        resolution_info: mockResolutionInfo('context')
      };
    } else if (key.includes('virtual')) {
      return {
        resolution_info: mockResolutionInfo('virtual')
      };
    }
    return null;
  };
  ...
  // Create mocks
  const resolveEntityMock = jest.fn().mockImplementation((type, name, options) => {
    if (resolverResults && resolverResults[`${type}_${name}`]) {
      return Promise.resolve(resolverResults[`${type}_${name}`]);
    }
    return Promise.resolve(defaultResolver(type, name));
  });
  
  const apiCacheGetMock = jest.fn().mockImplementation((key) => {
    if (resolverResults && resolverResults[`cache_${key}`]) {
      return resolverResults[`cache_${key}`];
    }
    return defaultCache(key);
  });
  
  const createMemoEqualityFnMock = jest.fn().mockImplementation(() => {
  ...
  const cleanup = () => {
    ...
    createMemoEqualityFnMock.mockClear();
  };
  
  return {
    resolveEntityMock,
    apiCacheGetMock,
    createMemoEqualityFnMock,
    cleanup
  };
};